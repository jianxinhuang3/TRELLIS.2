"""
Strict multi-step evaluation for T3 Tex flow single-sample overfitting.

Same protocol as eval_strict_t2.py (T2 Shape flow), but for the TEX flow
(InjectedSLatFlowModel with in_channels=64 / out_channels=32): the GT shape
latent is CONCATENATED to the model input at every denoising step
(concat_cond, matching the training-time kwargs flow: dataset provides
'concat_cond' -> trainer forwards it as a kwarg -> InjectedSLatFlowModel
does sparse_cat([x, concat_cond], dim=-1)).

Evaluates from PURE Gaussian noise on the GT coords Sg using manual
multi-step Euler sampling.

Metrics:
  - tex latent MSE (pred_feats vs gt_feats, normalized space; random
    baseline computed on the fly as mse(randn, gt_normalized))
  - gate=0 consistency check (max|delta| < 1e-3)
  - optional tex_dec + shape_dec decode + PBR rendering comparison
"""
import os
import sys
import json
...
def get_single_sample(dataset):
    """Get a single collated sample (SparseTensor x_0 + concat_cond)."""
    sample = dataset[0]
...
@torch.no_grad()
def sample_euler_manual_sparse(model, noise, cond, neg_cond, concat_cond, proxy_latent,
                               steps=50, guidance=1.0, cfg_mode='joint'):
    """
    Manual multi-step Euler sampling for the tex flow with proper CFG handling.

    Args:
        noise: SparseTensor with pure Gaussian feats (32ch) on GT coords Sg.
        cond / neg_cond: [B, Lc, 1024] DINOv3 tokens / zeros.
        concat_cond: SparseTensor with the GT shape latent (32ch, normalized)
            on the same coords Sg; concatenated to the input at EVERY step for
            BOTH the positive and negative CFG branches (same as training,
            where the trainer always forwards it as a kwarg).
        proxy_latent: [B, 8, 16, 16, 16] geometry condition.
        cfg_mode: 'joint' (neg drops proxy too) or 'standard' (neg keeps proxy).
    Returns:
        SparseTensor with the final sampled tex feats (32ch).
    """
    x_t = noise
    ...
    for t, t_prev in tqdm(t_pairs, desc=f'Euler (g={guidance}, {cfg_mode})'):
        ...
        if guidance == 1.0:
            pred_v = model(x_t, t_tensor, cond, concat_cond=concat_cond,
                           proxy_latent=proxy_latent)
        else:
            pred_pos = model(x_t, t_tensor, cond, concat_cond=concat_cond,
                             proxy_latent=proxy_latent)
            if cfg_mode == 'joint':
                pred_neg = model(x_t, t_tensor, neg_cond, concat_cond=concat_cond,
                                 proxy_latent=None)
            else:
                pred_neg = model(x_t, t_tensor, neg_cond, concat_cond=concat_cond,
                                 proxy_latent=proxy_latent)
...
@torch.no_grad()
def gate_zero_consistency_check(model, x_0, concat_cond):
    ...
    y_with = model(x, t, cond, concat_cond=concat_cond, proxy_latent=proxy_latent)
    y_without = model(x, t, cond, concat_cond=concat_cond, proxy_latent=None)
...
@torch.no_grad()
def render_comparison(dataset, pred, gt, concat_cond, save_dir, tag, title=''):
    """Decode pred/gt tex latents through tex_dec (guided by the GT shape
    latent via shape_dec) and save side-by-side PBR renders per output key."""
    import matplotlib
    matplotlib.use('Agg')
    ...
    for name, z in [('Predicted', pred), ('GT', gt)]:
        # SLatPbrVisMixin.visualize_sample expects {'x_0': tex_z, 'concat_cond': shape_z}
        images = dataset.visualize_sample({'x_0': z.cuda(), 'concat_cond': concat_cond.cuda()})
        renders[name] = {k: v[0].clamp(0, 1).permute(1, 2, 0).cpu().numpy()
                         for k, v in images.items()}

    saved = []
    for key in renders['Predicted'].keys():
        fig, axes = plt.subplots(1, 2, figsize=(16, 8))
        for ax, name in zip(axes, ['Predicted', 'GT']):
            ax.imshow(renders[name][key])
            ax.set_title(f'{name} ({key})')
            ax.set_axis_off()
        fig.suptitle(title, fontsize=11)
        fig.tight_layout()
        save_path = os.path.join(save_dir, f'vis_{tag}_{key}.png')
...
def main():
    parser = argparse.ArgumentParser(description='Strict multi-step evaluation for T3 Tex flow')
    ...
    parser.add_argument('--no_render', action='store_true', help='Skip tex_dec rendering')
    ...
    print(f'Base in_channels={model.base.in_channels}, out_channels={model.base.out_channels}')
    ...
    x_0 = batch['x_0'].cuda()                      # tex latent (32ch, normalized)
    concat_cond = batch['concat_cond'].cuda()      # GT shape latent (32ch, normalized)
    assert torch.equal(x_0.coords, concat_cond.coords), \
        'tex latent and shape latent must share the same coords Sg'
    print(f'x_0 (tex): tokens={x_0.feats.shape[0]}, dim={x_0.feats.shape[1]}, '
          f'feats std={x_0.feats.std().item():.4f}')
    print(f'concat_cond (shape): tokens={concat_cond.feats.shape[0]}, '
          f'dim={concat_cond.feats.shape[1]}')
    ...
    # Random baseline: pure noise feats vs gt feats (computed on the fly)
    ...
    for cfg_mode in cfg_modes:
        for guidance in guidances:
            ...
            pred = sample_euler_manual_sparse(
                model, noise, cond, neg_cond, concat_cond, proxy_latent,
    ...
    gate_check = gate_zero_consistency_check(model, x_0, concat_cond)
    print(f'Gate=0 check: {gate_check}')

    # Optional tex_dec rendering (guided by the GT shape latent)
    vis_paths = []
    if not args.no_render and best_pred is not None:
        try:
            vis_paths = render_comparison(
                dataset, best_pred, x_0, concat_cond, args.output_dir,
                tag=f'best_{best_config}_step{args.step}',
